classdef FlowComponent < IComponent
    ...
    methods
        ...
        function update_face_velocities(obj, vx, vr, aP_vx, aP_vr)
            [rhie_chow_vx, rhie_chow_vr] = rhie_chow_interpolation(obj.grid, vx, vr, obj.p, obj.vx_bds, obj.vr_bds, obj.p_bds, aP_vx, aP_vr, obj);
        ...
        function coeff_vr = get_coefficients_vr(obj, Fx, Fr, Dx, Dr)
            ...
            % Additional geometry term
            geom_term = 2 * pi * obj.grid.dx .* log(obj.grid.face_pos_r(1,2:end) ./ obj.grid.face_pos_r(1,1:end-1)) .* obj.visc / obj.porosity;
            geom_term(:,1) = 4 * pi * obj.grid.dx .* obj.visc(:,1) / obj.porosity; 
            coeff_vr(:,:,1) = coeff_vr(:,:,1) + geom_term;
        end
        ...
        function coeff_p_corr = get_coefficients_p_corr(obj, coeff_vx_aP, coeff_vr_aP)
            ...
            lower_face_r = [obj.grid.face_pos_r(1,1) obj.grid.cent_pos_r];
            upper_face_r = [obj.grid.cent_pos_r obj.grid.face_pos_r(1,end)];

            staggered_center_r = (lower_face_r + upper_face_r) / 2;

            volume_r = 2 * pi .* obj.grid.dx .* (upper_face_r - lower_face_r) .* staggered_center_r;

            % Removing the geometric term
            geom_term = 2 * pi * obj.grid.dx .* log(obj.grid.face_pos_r(1,2:end) ./ obj.grid.face_pos_r(1,1:end-1)) .* obj.visc / obj.porosity;
            geom_term(:,1) = 4 * pi * obj.grid.dx .* obj.visc(:,1) / obj.porosity;
            coeff_vr_aP = coeff_vr_aP - geom_term;
            ...
            [~, visc_r] = evaluate_faces(lin_int_coeffs_x, lin_int_coeffs_r, obj.visc);
            geom_term = 2 * pi * obj.grid.dx .* log(upper_face_r ./ lower_face_r) .* visc_r / obj.porosity;
            geom_term(:,1) = 4 * pi * obj.grid.dx .* visc_r(:,1) / obj.porosity;
            Df_r = Df_r .* volume_r ./ (volume_r + Df_r .* geom_term);

            clear geom_term lower_face_r upper_face_r staggered_center_r volume_r
            ...
            [coeff_der_p_x, coeff_der_p_r] = green_gauss_like_scheme(obj.grid, obj.p_corr_bds);
        ...
        function converged = iterate(obj)
            ...
            for i = 1:obj.inner_iters
                ...
                % Multiplying the pressure by face areas
                p_corr_x = p_corr_x .* obj.grid.face_area_x;
                p_corr_r = p_corr_r .* obj.grid.face_area_r;
                % Adding the volume integral of pressure gradient as source terms
                p_force_x = p_corr_x(1:end-1,:) - p_corr_x(2:end,:);
                p_force_r = p_corr_r(:,1:end-1) - p_corr_r(:,2:end);
